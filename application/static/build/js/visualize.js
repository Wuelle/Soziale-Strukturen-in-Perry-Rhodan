...
data.wheelSensitivity=0.1;cy=cytoscape(data);cy.ready((e)=>{bb=cy.bubbleSets();});cy.on("render",(e)=>{console.log("I am ready")})
...
else{node.removeClass("withLabel")}}}).trigger("change");$("#toggleLayout").change((e)=>{use_cola=e.target.checked;makeLayout().run();}).trigger("change");$("#cycle_selector").on("select2:select",async(e)=>{removeBubblesets();$("#communities").empty()
...
function removeBubblesets(){for(var path of bb.getPaths()){bb.removePath(path);}}
...
let cy_nodes=cy.collection();for(var char of chars){cy_nodes=cy_nodes.union(cy.nodes("#"+char)[0]);}
bb.addPath(cy_nodes,null,null,{style:{fill:colors_transparent[g_id]},stroke:"green"});let li=$("<li></li>").attr("id",g_id);let color_block=$("<span></span>").addClass("color_block").css({"background-color":colors[g_id]});li.append(color_block);li.append(" "+chars.length+" Mitglieder");$("#communities").append(li);}}
